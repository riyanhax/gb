...
$title = 'Ошибочные входы';
include ('../inc/head.php');

define('root', ROOT.'/panel/autherr.php');

if (isset($active) && $user['admin'] == 1) {
echo '<div class="mybar"><center>'.$title.'</center></div>';


$k_post = mysql_result(mysql_query("select count(*) from `auth_err` where `id`"),0); 
$k_page = k_page($k_post, 10); 
$page = page($k_page); 
$start = 10 * $page - 10;
			
$sql = mysql_query("select * from `auth_err` where `id` order by `id` asc limit ".$start.", 10");
...
&raquo; Логин: '.utf($row['log']).'<br/>Пароль: '.utf($row['pass']).'<br/>
Дата: '.date('d.m.Y H:i', $row['time']).'<br/></div>';}
