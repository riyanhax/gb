...
define('root', ROOT.'/panel/search.php');
...
echo '<div class="menu"><form action="search.php?act=view" method="POST">
Введите запрос:<br />
...
$k_post = mysql_num_rows(mysql_query("SELECT * FROM `users` WHERE `ip` LIKE '%".$text."%'"));
$k_page = k_page($k_post, 10); 
$page = page($k_page); 
$start = 10 * $page - 10; 
$sql = mysql_query("SELECT * FROM `users` WHERE `ip` LIKE '%".$text."%' ORDER BY `ip` DESC LIMIT ".$start.", 10");

if (mysql_num_rows($sql) == 0) {
echo '<div class="error"><center>В базе нет сведений!</center></div>';
} else {
$count = '';
while ($row = mysql_fetch_assoc($sql)) {
$count ++;
echo (is_integer($count / 2) ? '<div class="menu">' : '<div class="menu">').'
&raquo; Кто: '.utf($row['login']).'<br/>IP: '.utf($row['ip']).'<br/>
UA: '.utf($row['ua']).'<br/>Посл.вход: '.date('d.m.Y H:i:s', $row['lasttime']).'<br/></div>';
}
